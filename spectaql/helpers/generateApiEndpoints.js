module.exports = function (options) {var _options$data, _options$data2;
  ...
  map(
    (server) => {
      var _server$description, _server$url;
      const separator = server === servers[servers.length - 1] ? '' : '\n​\n'; // Adds an extra line between servers
      return (
        '# ' + (
        ((_server$description = server.description) === null || _server$description === void 0 ? void 0 : _server$description.trim()) || 'Endpoint') +
        ':\n' + (
        ((_server$url = server.url) === null || _server$url === void 0 ? void 0 : _server$url.trim()) || '<<url is missing>>') + separator
      );}
  ).
  join('');
};